...
- `mode`: `POST_MEETING`, `PRE_MEETING`, or `WEEKLY_REVIEW`
- `initiative_id`
- `workspace_path`
- `intent`
- source IDs or paths

Only access files under the supplied workspace path. The seeded knowledge base is in `data/seed/`. Canonical initiative memory is in `data/runtime/memory.json` when that file exists.

## Specialist workflow

Run these specialist roles in order. They are logical roles and may execute inside one agent turn.
...
- Every item in `what_changed`, `decisions`, `risks`, `open_questions`, `memory_proposals`, and `actions` must include `source_ids`.
...
```json
{
  ...
  "what_changed": [{"statement":"string","source_ids":["S1"]}],
  "decisions": [{"statement":"string","status":"CONFIRMED|PROPOSED|CONFLICTED","source_ids":["S1"]}],
  "commitments": [{"statement":"string","owner":"string|null","deadline":"string|null","source_ids":["S1"]}],
  "risks": [{"statement":"string","severity":"LOW|MEDIUM|HIGH|CRITICAL","source_ids":["S1"]}],
  "dependencies": [{"statement":"string","source_ids":["S1"]}],
  "open_questions": [{"statement":"string","source_ids":["S1"]}],
  "memory_proposals": [{"operation":"ADD|CONFIRM|DISPUTE|SUPERSEDE","type":"FACT|DECISION|COMMITMENT|RISK|DEPENDENCY|OPEN_QUESTION","statement":"string","supersedes_memory_id":"string|null","confidence":0.0,"source_ids":["S1"]}],
  "actions": [{"title":"string","description":"string","owner":"string|null","deadline":"string|null","status":"PROPOSED","human_assignment_required":false,"human_deadline_required":false,"source_ids":["S1"]}],
...
```json
{"meeting_brief":{"objective":"string","context":[{"statement":"string","source_ids":["S1"]}],"decisions_needed":["string"],"questions_to_ask":["string"]}}
```

Do not add keys containing hidden reasoning.
