...
export default function AskLuciPage({ params }: PageProps) {
  ...
  const targetRunId = useMemo(() => {
    ...
    return match?.id ?? runs.find((r) => r.initiativeId === initiativeId)?.id;
  }, [initiativeId, runType]);

  const handleStart = () => {
    if (!targetRunId) return;
    startTransition(() => {
      router.push(`/initiatives/${initiativeId}/runs/${targetRunId}`);
    });
  };
      ...
      <Panel className="space-y-5 p-4 sm:p-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
          ...
          <Button
            ...
            onClick={handleStart}
            disabled={isPending || selected.length === 0 || !targetRunId}
          >
